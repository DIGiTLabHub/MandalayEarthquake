...
for idx, row in df.iterrows():
    ...
    try:
        caption_prompt = (
            ...
            "At the beginning, include a short descriptive tag in parentheses.\n\n"
            "On a new line, output the list of relevant tags in the following exact Python list format:\n"
            "Tags: ['Damaged Building', 'People', 'Rescue', 'Debris', 'Injured People']"
        )
        ...
        if tag_line:
            ...
            try:
                ...
                print(f"⚠️ Warning: Couldn't parse tags for {image_path} with ast.literal_eval. Got: {raw_tags}")
                # Fallback: attempt to split by commas if not a valid list literal
                tags = [tag.strip() for tag in raw_tags.strip("[]").split(",") if tag.strip()]
    except Exception as e:
    ...
    # --- Stats Tracking ---
    for tag in tags:
        if "damage" in tag.lower():
            image_tags["Damaged Building"] += 1
        if "injur" in tag.lower() or "death" in tag.lower():
            image_tags["Injury/Death"] += 1
        if "rescue" in tag.lower() or "recovery" in tag.lower():
            image_tags["Recovery"] += 1
    if "damage" in summary_text.lower():
        summary_tags["Damaged Building"] += 1
    if "injur" in summary_text.lower() or "death" in summary_text.lower():
        summary_tags["Injury/Death"] += 1
    if "rescue" in summary_text.lower() or "recovery" in summary_text.lower():
        summary_tags["Recovery"] += 1
